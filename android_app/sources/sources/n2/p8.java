package n2;

import n2.i0;
import n2.p1;
import n2.x0;

/* JADX INFO: loaded from: classes.dex */
public class p8 extends ia {
    public p8() {
    }

    @Override // n2.i0
    public f C1() {
        if (!this.X) {
            f fVarC1 = super.C1();
            fVarC1.D(2);
            fVarC1.F(2);
            return fVarC1;
        }
        f fVarC12 = super.C1();
        fVarC12.D(2);
        fVarC12.F(3);
        fVarC12.C(1);
        fVarC12.d(true, 24);
        return fVarC12;
    }

    @Override // n2.i0
    public i0 M0() {
        p8 p8Var = new p8(this.f9523c);
        p8Var.e2(this.X);
        return p8Var;
    }

    @Override // n2.ja
    public int b2(int i4, boolean z3) {
        if (this.X) {
            return (i4 == 0 || i4 == 1) ? 66 : 100;
        }
        if (i4 == 0 || i4 == 1) {
            return 131;
        }
        return i4 == 7 ? 109 : 196;
    }

    @Override // n2.ia, n2.p1
    public boolean c() {
        return true;
    }

    @Override // n2.ia
    public void e2(boolean z3) {
        i0.a aVar;
        super.e2(z3);
        if (z3) {
            p1.a aVar2 = this.f9527g;
            aVar2.f9531b = 0.0f;
            aVar2.f9532c = 1.3962634f;
            aVar2.f9533d = false;
            aVar2.f9535f = 128;
            aVar2.f9536g = 512;
            int[] iArr = aVar2.f9537h;
            iArr[0] = 512;
            iArr[1] = 512;
            iArr[2] = 512;
            iArr[3] = 512;
            int[] iArr2 = aVar2.f9538i;
            iArr2[0] = 90;
            iArr2[1] = 130;
            iArr2[2] = 160;
            iArr2[3] = 200;
            aVar2.f9539j = l.a().c(4);
            p1.a aVar3 = this.f9527g;
            aVar3.f9540k = 4.0E7f;
            int[] iArr3 = aVar3.f9541l;
            iArr3[0] = 9;
            iArr3[1] = 13;
            iArr3[2] = 16;
            iArr3[3] = 20;
            x0.a aVar4 = this.f9683s;
            aVar4.f9691b = 0;
            aVar4.f9692c = 1;
            float[][] fArr = aVar4.f9693d;
            fArr[0][0] = 50.0f;
            fArr[1][0] = 50.0f;
            fArr[2][0] = 80.0f;
            fArr[3][0] = 80.0f;
            aVar4.f9694e = 1;
            float[][] fArr2 = aVar4.f9695f;
            fArr2[0][0] = 50.0f;
            fArr2[1][0] = 50.0f;
            fArr2[2][0] = 80.0f;
            fArr2[3][0] = 80.0f;
            aVar4.f9696g = 3.6f;
            aVar4.f9697h = 4.0f;
            aVar = this.A;
            aVar.f9355b = 0;
            aVar.f9356c = 4;
            float[] fArr3 = aVar.f9357d;
            fArr3[0] = 30.0f;
            fArr3[1] = 50.0f;
            fArr3[2] = 80.0f;
            fArr3[3] = 80.0f;
            aVar.f9358e = 20;
            aVar.f9359f = 16;
            aVar.f9360g = (aVar3.f9536g * 2) / 5;
            aVar.f9361h = 16;
            aVar.f9362i = 64;
            aVar.f9363j = 4;
            int[] iArr4 = aVar.f9364k;
            iArr4[0] = 2500;
            iArr4[1] = 3200;
            iArr4[2] = 4000;
            iArr4[3] = 5000;
            aVar.f9365l = 2.5f;
            aVar.f9366m = 16;
            aVar.f9367n = 1;
            aVar.f9368o[0] = 0;
            aVar.f9369p = true;
            aVar.f9370q = 4;
            int[] iArr5 = aVar.f9371r;
            iArr5[0] = 50;
            iArr5[1] = 50;
            iArr5[2] = 80;
            iArr5[3] = 80;
            aVar.f9372s = 4;
            int[] iArr6 = aVar.f9373t;
            iArr6[0] = 2000;
            iArr6[1] = 2500;
            iArr6[2] = 3200;
            iArr6[3] = 4000;
            aVar.f9374u = 8;
            aVar.f9375v = 2.5f;
            aVar.f9376w = 1;
            aVar.f9377x[0] = 0;
            aVar.f9378y = 3;
            int[] iArr7 = aVar.f9379z;
            iArr7[0] = 255;
            iArr7[1] = 128;
            iArr7[2] = 64;
        } else {
            p1.a aVar5 = this.f9527g;
            aVar5.f9531b = 60.0f;
            aVar5.f9532c = 1.0471976f;
            aVar5.f9533d = false;
            aVar5.f9535f = 128;
            aVar5.f9536g = 512;
            int[] iArr8 = aVar5.f9537h;
            iArr8[0] = 512;
            iArr8[1] = 512;
            iArr8[2] = 512;
            iArr8[3] = 512;
            int[] iArr9 = aVar5.f9538i;
            iArr9[0] = 90;
            iArr9[1] = 160;
            iArr9[2] = 240;
            iArr9[3] = 305;
            aVar5.f9539j = l.a().c(4);
            p1.a aVar6 = this.f9527g;
            aVar6.f9540k = 4.0E7f;
            int[] iArr10 = aVar6.f9541l;
            iArr10[0] = 9;
            iArr10[1] = 16;
            iArr10[2] = 24;
            iArr10[3] = 31;
            x0.a aVar7 = this.f9683s;
            aVar7.f9691b = 0;
            aVar7.f9692c = 1;
            float[][] fArr4 = aVar7.f9693d;
            fArr4[0][0] = 20.0f;
            fArr4[1][0] = 50.0f;
            fArr4[2][0] = 70.0f;
            fArr4[3][0] = 100.0f;
            aVar7.f9694e = 1;
            float[][] fArr5 = aVar7.f9695f;
            fArr5[0][0] = 40.0f;
            fArr5[1][0] = 60.0f;
            fArr5[2][0] = 80.0f;
            fArr5[3][0] = 100.0f;
            aVar7.f9696g = 3.2f;
            aVar7.f9697h = 4.0f;
            aVar = this.A;
            aVar.f9355b = 0;
            aVar.f9356c = 4;
            float[] fArr6 = aVar.f9357d;
            fArr6[0] = 30.0f;
            fArr6[1] = 60.0f;
            fArr6[2] = 80.0f;
            fArr6[3] = 100.0f;
            aVar.f9358e = 20;
            aVar.f9359f = 16;
            aVar.f9360g = (aVar6.f9536g * 2) / 5;
            aVar.f9361h = 16;
            aVar.f9362i = 64;
            aVar.f9363j = 4;
            int[] iArr11 = aVar.f9364k;
            iArr11[0] = 1250;
            iArr11[1] = 1600;
            iArr11[2] = 2500;
            iArr11[3] = 3200;
            aVar.f9365l = 2.5f;
            aVar.f9366m = 16;
            aVar.f9367n = 1;
            aVar.f9368o[0] = 0;
            aVar.f9369p = true;
            aVar.f9370q = 4;
            int[] iArr12 = aVar.f9371r;
            iArr12[0] = 30;
            iArr12[1] = 60;
            iArr12[2] = 80;
            iArr12[3] = 100;
            aVar.f9372s = 4;
            int[] iArr13 = aVar.f9373t;
            iArr13[0] = 1250;
            iArr13[1] = 1600;
            iArr13[2] = 2000;
            iArr13[3] = 2500;
            aVar.f9374u = 8;
            aVar.f9375v = 2.5f;
            aVar.f9376w = 1;
            aVar.f9377x[0] = 0;
            aVar.f9378y = 3;
            int[] iArr14 = aVar.f9379z;
            iArr14[0] = 255;
            iArr14[1] = 128;
            iArr14[2] = 64;
        }
        aVar.A = true;
        x();
    }

    @Override // n2.p1
    public boolean k() {
        return true;
    }

    @Override // n2.i0, n2.x0, n2.b1, n2.p1
    public void x() {
        super.x();
        S1(40);
        k0(false);
        o1 o1VarD = o1.d(null);
        this.D = o1VarD.e(n1(), (this.f9527g.f9535f / 2) - 30);
        this.E = o1VarD.e(m1(), (this.f9527g.f9535f / 2) + 30);
        this.O = o1VarD.e(x1(), this.f9527g.f9535f / 2);
    }

    @Override // n2.ia, n2.ja, n2.i0, n2.x0, n2.b1, n2.p1
    public r0 y() {
        ga gaVar = (ga) super.y();
        gaVar.P1(this.f9527g.f9535f, 32);
        return gaVar;
    }

    public p8(String str) {
        super(str);
        A("USC");
        p1.a aVar = this.f9527g;
        aVar.f9531b = 60.0f;
        aVar.f9532c = 1.0471976f;
        aVar.f9533d = false;
        aVar.f9535f = 128;
        aVar.f9536g = 512;
        int[] iArr = aVar.f9537h;
        iArr[0] = 512;
        iArr[1] = 512;
        iArr[2] = 512;
        iArr[3] = 512;
        int[] iArr2 = aVar.f9538i;
        iArr2[0] = 90;
        iArr2[1] = 160;
        iArr2[2] = 240;
        iArr2[3] = 305;
        aVar.f9539j = l.a().c(4);
        p1.a aVar2 = this.f9527g;
        aVar2.f9540k = 4.0E7f;
        int[] iArr3 = aVar2.f9541l;
        iArr3[0] = 9;
        iArr3[1] = 16;
        iArr3[2] = 24;
        iArr3[3] = 31;
        x0.a aVar3 = this.f9683s;
        aVar3.f9691b = 0;
        aVar3.f9692c = 1;
        float[][] fArr = aVar3.f9693d;
        fArr[0][0] = 20.0f;
        fArr[1][0] = 50.0f;
        fArr[2][0] = 70.0f;
        fArr[3][0] = 100.0f;
        aVar3.f9694e = 1;
        float[][] fArr2 = aVar3.f9695f;
        fArr2[0][0] = 40.0f;
        fArr2[1][0] = 60.0f;
        fArr2[2][0] = 80.0f;
        fArr2[3][0] = 100.0f;
        aVar3.f9696g = 3.2f;
        aVar3.f9697h = 4.0f;
        i0.a aVar4 = this.A;
        aVar4.f9355b = 0;
        aVar4.f9356c = 4;
        float[] fArr3 = aVar4.f9357d;
        fArr3[0] = 30.0f;
        fArr3[1] = 60.0f;
        fArr3[2] = 80.0f;
        fArr3[3] = 100.0f;
        aVar4.f9358e = 20;
        aVar4.f9359f = 16;
        aVar4.f9360g = (aVar2.f9536g * 2) / 5;
        aVar4.f9361h = 16;
        aVar4.f9362i = 64;
        aVar4.f9363j = 4;
        int[] iArr4 = aVar4.f9364k;
        iArr4[0] = 1250;
        iArr4[1] = 1600;
        iArr4[2] = 2500;
        iArr4[3] = 3200;
        aVar4.f9365l = 2.5f;
        aVar4.f9366m = 16;
        aVar4.f9367n = 1;
        aVar4.f9368o[0] = 0;
        aVar4.f9369p = true;
        aVar4.f9370q = 4;
        int[] iArr5 = aVar4.f9371r;
        iArr5[0] = 30;
        iArr5[1] = 60;
        iArr5[2] = 80;
        iArr5[3] = 100;
        aVar4.f9372s = 4;
        int[] iArr6 = aVar4.f9373t;
        iArr6[0] = 1250;
        iArr6[1] = 1600;
        iArr6[2] = 2000;
        iArr6[3] = 2500;
        aVar4.f9374u = 8;
        aVar4.f9375v = 2.5f;
        aVar4.f9376w = 1;
        aVar4.f9377x[0] = 0;
        aVar4.f9378y = 3;
        int[] iArr7 = aVar4.f9379z;
        iArr7[0] = 255;
        iArr7[1] = 128;
        iArr7[2] = 64;
        aVar4.A = true;
        z("{'abdomen':{'gain':80,'harmonic':0,'dr':80,'enhLevel':2,'focuspos':3},'gynecology':{'gain':80,'harmonic':0,'dr':80,'enhLevel':2,'focuspos':2},'obstetric':{'gain':80,'harmonic':0,'dr':80,'enhLevel':2,'focuspos':2},'cardiac':{'gain':80,'harmonic':1,'dr':80,'enhLevel':2,'focuspos':2},'urology':{'gain':80,'harmonic':1,'dr':80,'enhLevel':2,'focuspos':2},'kidney':{'gain':80,'harmonic':0,'dr':80,'enhLevel':2,'focuspos':2}}");
        x();
    }
}
